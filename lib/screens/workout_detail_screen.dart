...
class _WorkoutDetailScreenState extends State<WorkoutDetailScreen> {
        ...
        return Scaffold(
          ...
          body: Padding(
            ...
            child: Column(
              ...
              children: [
                const Text('Workout Notes', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18)),
                    ...
                    IconButton.filled(
                      onPressed: () {
                        if (_noteController.text.trim().isNotEmpty) {
                          provider.addNoteToWorkout(workout.id, _noteController.text.trim());
                          _noteController.clear();
                        }
                      },
                       ...
                       child: Row(
                         children: [
                           ...
                           IconButton(
                             ...
                             onPressed: () => _showEditNoteItemDialog(context, provider, workout.id, index, noteText),
                           ),
                           IconButton(
                             icon: const Icon(Icons.delete_outline, size: 18, color: Colors.red),
                             onPressed: () => provider.deleteNoteFromWorkout(workout.id, index),
                           ),
                         ],
                       ),
                     ),
                ...
                Expanded(
                  child: _buildHistoryList(provider, workout.id),
                )
              ],
            ),
          ),
  ...
  Widget _buildHistoryList(WorkoutProvider provider, String workoutId) {
    ...
    final sortedDates = historyByDate.keys.toList()..sort((a, b) => b.compareTo(a));
